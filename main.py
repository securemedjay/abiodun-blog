...
from flask_bootstrap import Bootstrap  # to readily use Bootstrap in your flask program and use Bootstrap HTML
# markup more easily e.g {{ wtf.quick_form(form)}}
from flask_ckeditor import CKEditor
from datetime import date
from werkzeug.security import generate_password_hash, check_password_hash  # to use and check password hash
from flask_sqlalchemy import SQLAlchemy  # to use python enabled SQL commands. Fields are object properties, tables are
# classes and each row of data is an object
...
# CONNECT TO DB
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv("DATABASE_URL", "sqlite:///blog.db")  # Connecting to the database.
# DATABASE_URL is an environment variable for POSTGRES db on the production server.
# ("DATABASE_URL", "sqlite:///blog.db") allows to run the code on sqlite (development server) if postgres in not
# available.

app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False  # silencing deprecation warning related to
# SQLALCHEMY_TRACK_MODIFICATIONS
db = SQLAlchemy(app)

# configuring flask app to use Flask_Login
...
class User(UserMixin, db.Model):  # User is the parent table
    __tablename__ = "users"
    id = db.Column(db.Integer, primary_key=True)  # declaring a column called id
...
db.create_all()  # creates all the databases
...
# creating a user loader function that is used to reload the user object from the user ID stored in the session
...
def add_new_post():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(new_post)  # adding a new record or role to the table
        db.session.commit()
        return redirect(url_for("get_all_posts"))
